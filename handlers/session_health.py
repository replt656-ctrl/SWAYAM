...
# AUTH_KEY_UNREGISTERED is deliberately excluded. Telegram can return it while
# a session is being reconnected after a process restart or while another
# client is still releasing the same authorization key. It must never cause
# this bot to disable or remove the stored account record automatically.
# AUTH_KEY_DUPLICATED is also excluded for the same reason.
        ...
        try:
            ...
            if _is_unregistered_key_error(error):
                return STATUS_UNVERIFIED, {
                    **info,
                    "error": (
                        "Telegram did not confirm this session during the "
                        "check; the account was kept unchanged."
                    ),
                }
